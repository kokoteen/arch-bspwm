...
WIDTH=200
WIDTH_WIDE=420
THEME=themes/kde_connect

# Color Settings of Icon shown in Polybar
COLOR_DISCONNECTED='#000'       # Device Disconnected
COLOR_NEWDEVICE='#ff0'          # New Device
COLOR_BATTERY_70='#fff'         # Battery >= 90
COLOR_BATTERY_60='#ccc'         # Battery >= 80
COLOR_BATTERY_50='#aaa'         # Battery >= 70
COLOR_BATTERY_40='#888'         # Battery >= 60
COLOR_BATTERY_30='#666'         # Battery >= 50
...
show_devices (){
    ...
    for device in $(qdbus --literal org.kde.kdeconnect /modules/kdeconnect org.kde.kdeconnect.daemon.devices); do
        ...
        then
            devices+="$(get_icon -1 "$devicetype")$SEPERATOR"   # disconnected
        else
            haspairing="$(qdbus org.kde.kdeconnect "/modules/kdeconnect/devices/$deviceid" org.kde.kdeconnect.device.hasPairingRequests)"
            if [ "$haspairing" = "true" ]
            then
                show_pmenu2 "$devicename" "$deviceid"   # pair request
            fi
            icon=$(get_icon -2 "$devicetype")       
            devices+="%{A1:$DIR/polybar-kdeconnect.sh -n $devicename -i $deviceid -p:}$icon%{A}$SEPERATOR"  # connect
...
show_menu () {
    menu="$(rofi -sep "|" -dmenu -i -p "$DEV_NAME" -theme $THEME -width $WIDTH -lines 6 <<< "Battery: $DEV_BATTERY%|Ping|Find Device|Send File|Browse Files|Unpair")"
                case "$menu" in
...
show_pmenu () {
    menu="$(rofi -sep "|" -dmenu -i -p "$DEV_NAME" -theme $THEME -width $WIDTH -lines 1 <<< "Pair Device")"
                case "$menu" in
...
show_pmenu2 () {
    menu="$(rofi -sep "|" -dmenu -i -p "$1 has sent a pairing request" -theme $THEME -width $WIDTH_WIDE -lines 2 <<< "Accept|Reject")"
                case "$menu" in
...
get_icon () {
    ...
    7*|8*|9*|100)   ICON="%{F$COLOR_BATTERY_70}$icon%{F-}" ;;
