...
[ ! -f "$HOME/.monitor_position" ] && echo -en "monitor_position 1\nmirror_mode 0" > $HOME/.monitor_position
...
if [ $(xrandr | grep -w connected | wc | awk '{print $1}') == "2" ]; then
	...
	if [ $(grep -w xinerama_head $FILE | awk -F'[, ]' '{print $3}') != $(grep -w monitor_position $HOME/.monitor_position | awk '{print $2}') ]; then
  		#left position
